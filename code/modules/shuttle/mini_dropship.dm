...
/obj/docking_port/stationary/som_dropship/calico
	name = "Calico hangar pad"
	id = SHUTTLE_SOMTADPOLE
	roundstart_template = null

/obj/docking_port/mobile/som_dropship/calico
	name = "Calico"
	id = SHUTTLE_SOMTADPOLE
	dwidth = 0
	dheight = 0
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico
	name = "Calico navigation computer"
	desc = "Used to designate a precise transit location for the Calico."
	icon_state = "shuttlecomputer"
	screen_overlay = "shuttlecomputer_screen"
	req_one_access = list(ACCESS_MARINE_DROPSHIP, ACCESS_MARINE_LEADER)
	density = FALSE
	...
	var/to_transit = TRUE
	/// The id of the stationary docking port on the ship
	var/origin_port_id = SHUTTLE_SOMTADPOLE
	/// The user of the ui
	var/mob/living/ui_user
	/// How long before you can launch tadpole after a landing
	var/launching_delay = 10 SECONDS
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/Initialize(mapload)
	..()
	start_processing()
	set_light(3,3, LIGHT_COLOR_RED)
	land_action = new
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/Destroy()
	QDEL_NULL(land_action)
	QDEL_NULL(tadmap)
	return ..()

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/CreateEye()
	. = ..()
	tadmap.override_locator(eyeobj)

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/LateInitialize()
	shuttle_port = SSshuttle.getShuttle(shuttleId)

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/give_actions(mob/living/user)
	if(!user)
		return
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/shuttle_arrived()
	if(fly_state == next_fly_state)
		return
	fly_state = next_fly_state
	if(fly_state == SHUTTLE_IN_SPACE)
...
///The action of taking off and sending the shuttle to the atmosphere
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/proc/take_off()
	shuttle_port = SSshuttle.getShuttle(shuttleId)
	#ifndef TESTING
	if(!(shuttle_port.shuttle_flags & GAMEMODE_IMMUNE) && world.time < SSticker.round_start_time + SSticker.mode.deploy_time_lock)
		to_chat(ui_user, span_warning("The mothership is too far away from the theatre of operation, we cannot take off."))
...
///The action of sending the shuttle back to its shuttle port on main ship
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/proc/return_to_ship()
	shuttle_port = SSshuttle.getShuttle(shuttleId)
	shuttle_port.shuttle_computer = src
	to_transit = TRUE
	next_fly_state = SHUTTLE_IN_SPACE
...
/// Toggle the vision between small nightvision and turf vision
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/proc/toggle_nvg()
	if(!check_hovering_spot(eyeobj?.loc))
		to_chat(ui_user, span_warning("Can not toggle night vision mode in caves"))
		return
	nvg_vision_mode = !nvg_vision_mode

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/attack_alien(mob/living/carbon/xenomorph/X, damage_amount, damage_type, damage_flag, effects, armor_penetration, isrightclick)
	. = ..()
	if(machine_stat & BROKEN)
		return
	if(X.status_flags & INCORPOREAL)
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/can_interact(mob/user)
	if(machine_stat & BROKEN)
		to_chat(user, span_warning("The [src] blinks and lets out a crackling noise. Its broken!"))
		return
	return ..()

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/ui_state(mob/user)
	return GLOB.dropship_state

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/ui_interact(mob/user, datum/tgui/ui)
	. = ..()
	ui = SStgui.try_update_ui(user, src, ui)
	if(ui_user)
		return
	...
	if(!ui)
		ui_user = user
		RegisterSignals(ui_user, list(COMSIG_QDELETING, COMSIG_MOVABLE_MOVED), PROC_REF(clean_ui_user))
		ui = new(user, src, "Calico", name)
		ui.open()

/obj/machinery/computer/camera_advanced/shuttle_docker/calico/ui_close(mob/user)
	. = ..()
	clean_ui_user()

/// Set ui_user to null to prevent hard del
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/proc/clean_ui_user()
	SIGNAL_HANDLER
	if(ui_user)
		remove_eye_control(ui_user)
		UnregisterSignal(ui_user, list(COMSIG_QDELETING, COMSIG_MOVABLE_MOVED))
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/ui_data(mob/user)
	. = list()
	if(!shuttle_port)
		shuttle_port = SSshuttle.getShuttle(shuttleId)
	.["fly_state"] = fly_state
	.["take_off_locked"] = ( !(fly_state == SHUTTLE_ON_GROUND || fly_state == SHUTTLE_ON_SHIP) || shuttle_port?.mode != SHUTTLE_IDLE)
	.["return_to_ship_locked"] = (fly_state != SHUTTLE_IN_ATMOSPHERE || shuttle_port?.mode != SHUTTLE_IDLE)
	var/obj/docking_port/mobile/som_dropship/shuttle = shuttle_port
	.["equipment_data"] = list()
	var/element_nbr = 1
	for(var/X in shuttle?.equipments)
		var/obj/structure/dropship_equipment/E = X
...
/obj/machinery/computer/camera_advanced/shuttle_docker/calico/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	switch(action)
		if("take_off")
			take_off()
...
/datum/action/innate/shuttledocker_land/Activate()
	var/mob/C = target
	var/mob/camera/aiEye/remote/remote_eye = C.remote_control
	var/obj/machinery/computer/camera_advanced/shuttle_docker/calico/origin = remote_eye.origin
	if(origin.shuttle_port.mode != SHUTTLE_IDLE)
		to_chat(owner, span_warning("The shuttle is not ready to land yet!"))
		return
	if(!origin.placeLandingSpot(target))
